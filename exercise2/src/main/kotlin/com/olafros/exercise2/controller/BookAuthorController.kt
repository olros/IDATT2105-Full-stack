...
class BookAuthorController(
    ...
    fun getBookAuthors(@PathVariable bookId: Long): ResponseEntity<List<AuthorDtoList>> {
        return ResponseEntity.ok(bookAuthorService.getBookAuthors(bookId))
    }
    ...
    fun createNewBookAuthor(
        ...
        return ResponseEntity.ok(bookAuthorService.createNewBookAuthor(bookId, newAuthor))
    }

    @DeleteMapping("/{authorId}")
    fun deleteBookAuthorById(@PathVariable bookId: Long, @PathVariable authorId: Long): ResponseEntity<*> {
        return ResponseEntity.ok(bookAuthorService.deleteBookAuthorById(bookId, authorId))
    }
}
